...
func (p *noOpPVolumeClient) GetPVMetrics() *PVMetrics {
	return &PVMetrics{
		ClusterCount: 0,
	}
}
...
func (p *PVolume) GetPVMetrics() *PVMetrics {
...
func newPVolumeClient(clientSet kubernetes.Interface, logger *zap.Logger, options ...PVolumeClientOption) (*PVolume, error) {
	...
	if _, err := clientSet.CoreV1().PersistentVolumes().List(ctx, metav1.ListOptions{}); err != nil {
		return nil, fmt.Errorf("cannot list PVs. err: %w", err)
	}

	// Create a store to hold PV objects
	p.store = NewObjStore(transformFuncPVolume, logger)
	// Create a ListWatch that knows how to list and watch PVs
	lw := createPVolumeListWatch(clientSet)
	// Create a Reflector that watches PVs and updates the store
